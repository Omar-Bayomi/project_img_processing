...
def stop_at_pos(Rover, mag, direction, max_mag):
            ...
            for i in range(len(Rover.nav_angles)):
                if abs(Rover.nav_angles[i]) < abs(Rover.nav_angles[angle_idx]):
                    angle_idx = i
            if (abs(direction) > 15 and mag < 0.5) or len(Rover.nav_dists)==0 or Rover.nav_dists[angle_idx] < 1:
                if Rover.vel > 0.2:
                    Rover.throttle = 0
                    Rover.brake = 0.1
                    if direction > 0:
...
def decision_step(Rover):
    ...
    if Rover.nav_angles is not None:
        ...
        eighted_angles = (Rover.nav_dists**n * Rover.nav_angles)
        weighted_angles.sort()
        print(Rover.terrain_width , Rover.max , Rover.min)
        #rover_width = 15
        if Rover.terrain_width > 60 and Rover.terrain_width < 90:
            weighted_angles = weighted_angles[0:int(len(weighted_angles)* (60/Rover.terrain_width))]
        elif Rover.terrain_width < 60:
            weighted_angles = weighted_angles[0:int(len(weighted_angles)*1)]
        else:
	...
	#############################################################################
        ...
        ###############################################
            ...
            if Rover.vel > 0.2:
                ...
                Rover.brake = 0
                if ((Rover.back_home and abs(Rover.yaw - Rover.backward_yaw) < 180) or (Rover.rock_found == True and (abs(Rover.yaw - Rover.backward_yaw) < 90))):
